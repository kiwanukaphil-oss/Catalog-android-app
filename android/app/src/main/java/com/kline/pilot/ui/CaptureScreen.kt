...
@Composable fun CaptureScreen(onClose: () -> Unit, onPhoto: (Uri) -> Unit) {
    ...
    DisposableEffect(permitted, front) {
        ...
        if (permitted) {
            ...
            future.addListener({
                ...
                }.onFailure { error = "Camera unavailable. You can import a photo instead." }
            }, ContextCompat.getMainExecutor(context))
        }
        onDispose { disposed = true; provider?.unbindAll() }
    }
    Column(Modifier.fillMaxSize().background(Color(0xFF102720)).safeDrawingPadding()) {
            ...
            Button(onClick = {
                ...
                capture.takePicture(ImageCapture.OutputFileOptions.Builder(file).build(), ContextCompat.getMainExecutor(context),
                    object : ImageCapture.OnImageSavedCallback {
                        override fun onImageSaved(output: ImageCapture.OutputFileResults) { busy = false; onPhoto(Uri.fromFile(file)) }
                        override fun onError(exception: ImageCaptureException) { busy = false; error = "Could not capture. Check storage and try again." }
                    })
